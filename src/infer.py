...
def plot_distributions(
    ...
    same_sim_label = (
        f"Same Class (μ={same_sims.mean():.4f}, σ={same_sims.std():.4f}, "
        f"Worst={worst_same_sim:.4f})"
    )
    diff_sim_label = (
        f"Different Class (μ={diff_sims.mean():.4f}, σ={diff_sims.std():.4f}, "
        f"Worst={worst_diff_sim:.4f})"
    )
    ...
    same_ham_label = (
        f"Same Class (μ={same_ham.mean():.4f}, σ={same_ham.std():.4f}, "
        f"Worst={worst_same_ham:.4f})"
    )
    diff_ham_label = (
        f"Different Class (μ={diff_ham.mean():.4f}, σ={diff_ham.std():.4f}, "
        f"Worst={worst_diff_ham:.4f})"
    )
...
def main():
    ...
    worst_same_class_sim = np.min(same_class_sims)
    worst_diff_class_sim = np.max(diff_class_sims)

    worst_same_class_ham = np.max(same_class_ham)
    worst_diff_class_ham = np.min(diff_class_ham)

    print("\n--- Performance Metrics ---")
    print(f"Worst Same-Class Cosine Similarity: {worst_same_class_sim:.4f}")
    print(f"Worst Different-Class Cosine Similarity: {worst_diff_class_sim:.4f}")
    print(f"Worst Same-Class Hamming Distance: {worst_same_class_ham:.4f}")
    print(f"Worst Different-Class Hamming Distance: {worst_diff_class_ham:.4f}")
    print("---------------------------\n")

    plot_distributions(
        same_class_sims,
        diff_class_sims,
        worst_same_class_sim,
        worst_diff_class_sim,
        same_class_ham,
        diff_class_ham,
        worst_same_class_ham,
        worst_diff_class_ham,
