...
def generate_and_plot_histograms(
    ...
    worst_cos_sim = -1.0
    worst_cos_pair_info = ("N/A", "N/A")
    if diff_class_sims_cos.size > 0:
        worst_cos_sim = diff_class_sims_cos.max()
        diff_indices = torch.triu_indices(num_samples, num_samples, offset=1)
        diff_mask_flat = mask_diff[diff_indices[0], diff_indices[1]]
        cos_sims_diff_flat = sim_matrix_cos[diff_indices[0], diff_indices[1]]
        worst_idx = torch.argmax(cos_sims_diff_flat[diff_mask_flat])

        valid_diff_indices = diff_indices[:, diff_mask_flat]
        worst_i, worst_j = valid_diff_indices[:, worst_idx]

        path_i = all_paths[worst_i]
        path_j = all_paths[worst_j]
        dir_i = os.path.basename(os.path.dirname(path_i))
        dir_j = os.path.basename(os.path.dirname(path_j))
        file_i = os.path.basename(path_i)
        file_j = os.path.basename(path_j)

        worst_cos_pair_info = (f"{dir_i}/{file_i}", f"{dir_j}/{file_j}")
    ...
    worst_ham_dist = float("inf")
    worst_ham_pair_info = ("N/A", "N/A")
    if diff_class_sims_ham.size > 0:
        worst_ham_dist = diff_class_sims_ham.min()
        ham_sims_diff_flat = sim_matrix_ham[diff_indices[0], diff_indices[1]]
        worst_idx = torch.argmin(ham_sims_diff_flat[diff_mask_flat])
        ...
        worst_ham_pair_info = (f"{dir_i}/{file_i}", f"{dir_j}/{file_j}")
    ...
    stats_text_cos = (
        ...
        f"Worst Diff Pair (Highest Sim):\n  Value: {worst_cos_sim:.4f}\n"
        f"  Pair: {worst_cos_pair_info[0]}\n         {worst_cos_pair_info[1]}"
    )
    ax1.text(
        0.02,
        0.98,
    ...
    stats_text_ham = (
        ...
        f"Worst Diff Pair (Lowest Dist):\n  Value: {worst_ham_dist:.4f}\n"
        f"  Pair: {worst_ham_pair_info[0]}\n         {worst_ham_pair_info[1]}"
    )
    ax2.text(
        0.02,
        0.98,
