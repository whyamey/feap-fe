...
def generate_and_plot_histograms(
    ...
    worst_cos_sim, worst_cos_pair_info = -1.0, ("N/A", "N/A")
    if diff_class_sims_cos.size > 0:
        worst_cos_sim = diff_class_sims_cos.max()
        worst_indices = (sim_matrix_cos == worst_cos_sim).nonzero(as_tuple=False)
        if len(worst_indices) > 0:
            for idx_pair in worst_indices:
                if idx_pair[0] != idx_pair[1]:
                    worst_i, worst_j = idx_pair
                    path_i, path_j = all_paths[worst_i], all_paths[worst_j]
                    worst_cos_pair_info = (
                        f"{os.path.basename(os.path.dirname(path_i))}/{os.path.basename(path_i)}",
                        f"{os.path.basename(os.path.dirname(path_j))}/{os.path.basename(path_j)}",
                    )
                    break
    ...
    worst_ham_dist, worst_ham_pair_info = float("inf"), ("N/A", "N/A")
    if diff_class_sims_ham.size > 0:
        worst_ham_dist = diff_class_sims_ham.min()
        worst_indices = (sim_matrix_ham == worst_ham_dist).nonzero(as_tuple=False)
        if len(worst_indices) > 0:
            for idx_pair in worst_indices:
                if idx_pair[0] != idx_pair[1]:
                    worst_i, worst_j = idx_pair
    ...
    stats_text_cos = (
        ...
        f"Worst Diff Sim: {worst_cos_sim:.4f}\n{worst_cos_pair_info[0]}\n{worst_cos_pair_info[1]}"
    )
    ax1.text(
        0.02,
        0.98,
    ...
    stats_text_ham = (
        ...
        f"Worst Diff Dist: {worst_ham_dist:.4f}\n{worst_ham_pair_info[0]}\n{worst_ham_pair_info[1]}"
    )
    ax2.text(
        0.02,
        0.98,
    ...
    return {
        ...
        "worst_cos_val": worst_cos_sim,
        "worst_cos_pair": f"{worst_cos_pair_info[0]} vs {worst_cos_pair_info[1]}",
        ...
        "worst_ham_val": worst_ham_dist,
        "worst_ham_pair": f"{worst_ham_pair_info[0]} vs {worst_ham_pair_info[1]}",
    }
...
def generate_and_plot_refiner_histograms(
    ...
    worst_dist, worst_pair_info = float("inf"), ("N/A", "N/A")
    if diff_class_dists.size > 0:
        worst_dist = diff_class_dists.min()
        ham_sims_diff_flat = ham_dist_matrix[
            valid_diff_indices[0], valid_diff_indices[1]
        ]
        worst_idx_in_flat = torch.argmin(ham_sims_diff_flat)
        worst_i, worst_j = valid_diff_indices[:, worst_idx_in_flat]

        path_i, path_j = eval_paths[worst_i], eval_paths[worst_j]
        worst_pair_info = (
            f"{os.path.basename(os.path.dirname(path_i))}/{os.path.basename(path_i)}",
            f"{os.path.basename(os.path.dirname(path_j))}/{os.path.basename(path_j)}",
        )
    ...
    stats_text = (
        ...
        f"Worst Diff Pair (Lowest Dist):\n  Value: {worst_dist:.4f}\n"
        f"  Pair: {worst_pair_info[0]}\n        {worst_pair_info[1]}"
    )
    plt.text(
        0.98,
        0.98,
    ...
    return {
        ...
        "worst_ham_val_refined": worst_dist,
        "worst_ham_pair_refined": f"{worst_pair_info[0]} vs {worst_pair_info[1]}",
    }
...
def run_full_training_pipeline(
    ...
    with open(refiner_csv_path, "w", newline="") as refiner_csv_file:
        ...
        refiner_header = [
            ...
            "worst_ham_val_refined",
            "worst_ham_pair_refined",
        ]
